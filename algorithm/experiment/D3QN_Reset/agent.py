...
class Agent():
    def __init__(self, algorithm):
        ...
        self.reset_steps = config['reset_interval']
        self.alg = algorithm
    ...
    def sample(self, state):
        ...
        if sample < self.exploration:
            action = np.random.randint(self.act_dim, size=config['num_envs'])
        else:
    ...
    def drop_reset(self, p=0.01):
        if self.global_step % self.reset_steps == 0:
            self.alg.drop_reset(p)
